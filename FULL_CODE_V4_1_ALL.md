# FULL CODE V4.1.0 — GM Character + 24H Chat
    ...
    <section id="registerPanel" class="auth-panel hidden">
      ...
      <form id="registerForm" class="form-grid">
        ...
        <label>
          ...
          <input id="fullName" required placeholder="เช่น นายสมชาย ใจดี">
        </label>
...
<body class="admin-page">
  <main class="wrap admin-wrap">
    ...
    <section id="adminDashboard" class="hidden">
      <div class="admin-titlebar">
        ...
        <div>
          <a class="btn gm-zone-entry" href="./zone.html?v=4.1.0">🌙 เข้า 2D Zone · GM</a>
      ...
      <section id="zoneChatLogTab" class="admin-tab-panel hidden">
        ...
        <div class="admin-zone-chat-summary">
          ...
          <span><b id="zoneChatTotalMetric">0</b> แสดงทั้งหมด</span>
        </div>
  ...
  <script type="module" src="./admin.js?v=4.1.0"></script>
</body>
</html>
...
<body class="zone-v41-page">
  <div id="zoneGate" class="zone-v40-gate">
    <div class="zone-v40-gate-card">
      ...
      <span class="section-kicker">2D ZONE · V4.1.0</span>
  ...
  <script>
  (function(){
    window.__ZONE_V41_BOOTED__ = false;
    ...
    function fail(message){
      ...
      if(h()){
        ...
        h().innerHTML="<strong>ตรวจไฟล์ V4.1.0</strong><p>อัปโหลดไฟล์ทั้งชุดและ Publish firestore.rules เวอร์ชันเดียวกัน</p>";
      }
    }
    window.addEventListener("error",e=>{if(!window.__ZONE_V41_BOOTED__)fail("โหลด 2D Zone ไม่สำเร็จ: "+(e.message||"JavaScript error"));});
    window.addEventListener("unhandledrejection",e=>{if(!window.__ZONE_V41_BOOTED__)fail("เปิด 2D Zone ไม่สำเร็จ: "+(e.reason?.message||e.reason?.code||"Promise error"));});
    setTimeout(()=>{if(!window.__ZONE_V41_BOOTED__)fail("2D Zone ใช้เวลานานผิดปกติ — ตรวจ zone.js และ Firebase Rules");},9000);
  })();
  </script>
  <script type="module" src="./zone.js?v=4.1.0"></script>
</body>
</html>
```
  ...
  match /databases/{database}/documents {
    ...
    match /pvp_rooms/{roomCode} {
      ...
      allow update: if signedIn() && (
        request.auth.uid in resource.data.players || request.auth.uid == resource.data.hostUid || request.auth.uid in request.resource.data.players
      );
